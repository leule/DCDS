...
def main(args):
    ...
    embed_model = [VNetEmbed(instances_num=args.num_instances,
                            feat_num=(2048 / grp_num), num_classes=2,
                            drop_ratio=args.dropout).cuda() for i in range(grp_num)]

    base_model = nn.DataParallel(base_model).cuda()

    model = VNetExtension(instances_num=args.num_instances, # 
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        top1, mAP = evaluator.evaluate(val_loader, dataset.val, dataset.val, args.alpha,rerank_topk=args.rerank, second_stage=True, dataset=args.dataset)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--rerank', type=int, default=70)
    ...
    parser.add_argument('--ss', type=int, default=10,
